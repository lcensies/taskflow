...
- Each phase runs as an isolated `grok -p --output-format streaming-json`
  ...
  in `.grok/sandbox.toml` or `~/.grok/sandbox.toml`; built-in profiles are
  rejected because Grok may warn and continue unsandboxed when enforcement is
  unavailable. The custom profile then runs with `--always-approve`. A
  `max_turns_reached` event fails the phase rather than returning partial text.
  Current terminal events may report observed tokens, turns, and complete USD
  cost, which unbudgeted runs retain. Older, partial, or incomplete events may
  omit them, so absence remains unknown rather than free. The runner therefore
  still advertises usage accounting as unavailable, and its MCP adapter rejects
  every flow with `budget` rather than pretending to enforce an unobservable
  threshold.
